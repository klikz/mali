...
function sort_sub_category(params) {
    for (let key in packing_type){
        if (packing_type[key] == true){
            sub_selected = true
            return
        }else{
            sub_selected = false
...
function search_text(lang) {
    let text = document.querySelector('#search_area').value
    window.location.href = '/search/'+ lang + "/?text="+text
}

async function send_message(lang){
...
fetch( '/message', options )
    ...
    .then( response => {
        ...
        lang == "en" ? document.getElementById("program").innerText  = "Feedback. Message send." : lang == "uz" ? document.getElementById("program").innerText  = "Fikr-mulohaza. Xabaringiz yuborildi." : document.getElementById("program").innerText  = "Обратная связь. Ваше сообщение отправлено." 
...
function shareURL() 
{ const url = window.location.href; if (navigator.share) { navigator.share({ title: document.title, url: url }).then(() => { 
    console.log('Thanks for sharing!'); }).catch(console.error); } else { 
    // Fallback for browsers that don't support the Web Share API prompt('Copy this URL to share:', url); 
    } }

function sub_category_change(params) {
    packing_type[params] = !packing_type[params]
    sort_sub_category(params)
    sortCategory()
    

}

function start(id_type) {
    id_type_g = id_type
    "use strict";
    function e(e) {
        ...
        if (t !== window.location.origin + "/" && t !== window.location.origin)
            if ("ru" === e)
                if (s.test(t)) {
                    ...
                    n.test(t) ? window.location.href = t : window.location.href = t.replace(/\/$/, "") + "";
            else {
                let a;
                a = s.test(t) ? t.replace(s, `-${e}`) : n.test(t) ? t.replace(n, `-${e}`) : i.test(t) ? t.replace(i, `-${e}`) : t.replace(/\/$/, `-${e}`),
                window.location.href = a
            }
        else
            window.location.href = "ru" === e ? "/ru" : `/${e}`
    }
    let lang = Z.split("/")
    ...
    function() {
        ...
        if (e.includes("detail/ru") || e.includes("detail/uz") || e.includes("detail/en")) {
    ...
    ("/"===Z||Z.includes("ru")||Z.includes("detail/ru")||"/uz"===Z||Z.includes("en")||Z.includes("detail/ru")||Z.includes("detail/uz")) && fetch("/files/products.json").then((e => e.json())).then((e => {
        ...
        e.forEach((e => {
            ...
            i = e.count ? "en" === ee ? e.weight.map((t => `
                <h3>${t} g / ${e.count} pcs</h3>`)).join("") : "uz" === ee ? e.weight.map((t => `
                    <h3>${t} g / ${e.count} dona</h3>`)).join("") : e.weight.map((t => `
                        <h3>${t} г / ${e.count} шт</h3>`)).join("") : e.weight.map((e => `
                            <h3>${e} ${"en" === ee || "uz" === ee ? "g" : "г"}</h3>`)).join(""),
            n = Z.includes("en") ? 
                `/detail/en?id=${e.id}` : Z.includes("uz") ? 
                    `/detail/uz?id=${e.id}` : `/detail/ru/?id=${e.id}`,
                    s.innerHTML = `\n                    
...
function sortCategory() {
    ...
    listPro = listPro.filter((list_1) => {
        if (id_type_g) {
            if (list_1.id_type == id_type_g) {
                if (sub_selected) {
                    if (packing_type[list_1.packing_type] == true) {
                        return true
                    } else {
                        return false
                    }
    ...
    listPro.forEach((e => {
        ...
        let i, n;
        // console.log("count: ", e.count, " ", e.count ? "en" === ee1 : "false"),
        s.classList.add("catalog__item"),
            i = e.count ? "en" === ee1 ? e.weight.map((t => `
                <h3>${t} g / ${e.count} pcs</h3>`)).join("") 
                : "uz" === ee1 ? e.weight.map((t =>
                `<h3>${t} g / ${e.count} dona</h3>`)).join("") 
                : e.weight.map((t =>
                    `<h3>${t} г / ${e.count} шт</h3>`)).join("") 
                    : e.weight.map((e =>
                        `<h3>${e} г</h3>`)).join(""),
            n = Z1.includes("/en") ? `detail/en?id=${e.id}` : Z1.includes("/uz") ? `detail/uz?id=${e.id}` : `detail/ru?id=${e.id}`,
            ...
            s.innerHTML = `\n                  
                ...
                <img src="${"/img/products/" + e.img}" alt="${e?.name?.[ee1]}" class="active">\n
                </div>\n                      
                ...
                <h3 class="catalog__item-title">${e?.type?.[ee1] ? e?.type?.[ee1] : ""} «${e?.name?.[ee1]}»</h3>\n                      
                <p class="catalog__item-text">${e.info[ee1]}</p>\n                      
...
function sort_cat(id_type){
    id_type_g = id_type
    sortCategory(id_type)
}